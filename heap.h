...
class heap
{
  public:
    ...
    /**
     * This function allows you to change the weights of each T item.
     * This will be used to ensure that any reordering will have
     * the appropriate, updated weight.
     * @param item - The item that we want to update
     * @param newWeight - the new weight associated with the item
     */
    void changeWeight(T item, double newWeight);

    /**
     * This establishes the weights used to compare items of type T.
     * @param weights - the new map to use
     */
    void addWeights(map<T, double> weights);
  ...
  private:
    ...
    /**
     * This will map all type T to its weight, where the weight
     * is what will be used to determine how each T will be ordered.
     */
